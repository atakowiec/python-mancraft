...
class Game:
    def __init__(self, game, world_id):
        self.game = game
    ...
    def general_update(self):
        # TO co dzialo sie w for
        for event in pygame.event.get():
            # TODO repair this fucking loop
            ...
            self.single_mouse_click = None
            if event.type == pygame.MOUSEBUTTONDOWN:
                self.single_mouse_click = event.button
    ...
    def update(self):
        ...
        if self.mouse_click[0] and self.clicked_block is not None and line_length <= self.player.range_of_hand * 20:
            if self.clicked_block == last_block:
                if self.world.blocks[self.clicked_block[0]][self.clicked_block[1]]:
                    ...
                    breaking_time = \
                        self.world.block_types[self.world.blocks[self.clicked_block[0]][self.clicked_block[1]]][1]
        ...
        # self.screen.blit(self.font.render(f"FPS: {round((1000 / self.TICK) * 30 / ((time.time() - timer) * 1000), 1)}, {(time.time() - timer)}", False, (0, 0, 0)), (10, 10))
    ...
    def load_world(self):
        if self.world_id != 0 and self.world_id is not None and os.path.exists(os.path.join(self.game.GAME_PATH, f"saves/{self.world_id}")):
